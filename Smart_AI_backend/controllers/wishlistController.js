...
const addToWishlist = asyncHandler(async (req, res) => {
  ...
  if (!productId) {
    throw new BadRequestError('productId là bắt buộc', 'INVALID_PRODUCT_ID');
  }
  ...
  if (existingItem) {
    throw new BadRequestError('Sản phẩm đã có trong wishlist', 'ALREADY_IN_WISHLIST');
  }
...
const removeFromWishlist = asyncHandler(async (req, res) => {
  ...
  let wishlist = await Wishlist.findOne({ user: req.user._id });
  if (!wishlist) {
    throw new NotFoundError('Không tìm thấy wishlist', 'NOT_IN_WISHLIST');
  }
  ...
  if (itemIndex === -1) {
    throw new NotFoundError('Sản phẩm không có trong wishlist', 'NOT_IN_WISHLIST');
  }
...
// Requirements: 7.1
const checkMultipleWishlistStatus = asyncHandler(async (req, res) => {
  const { productIds } = req.body;

  if (!productIds || !Array.isArray(productIds)) {
    throw new BadRequestError('productIds phải là một mảng', 'INVALID_PRODUCT_IDS');
  }
