...
const validatePromotion = asyncHandler(async (req, res) => {
  ...
  if (!code) {
    throw new BadRequestError('Vui lòng nhập mã khuyến mãi');
  }
  ...
  if (!promotion) {
    throw new NotFoundError('Mã khuyến mãi không hợp lệ');
  }

  // Check if promotion is active
  if (!promotion.isActive) {
    throw new BadRequestError('Mã khuyến mãi không còn hiệu lực');
  }
  ...
  if (promotion.startDate > now) {
    throw new BadRequestError('Mã khuyến mãi chưa có hiệu lực');
  }

  // Check if promotion has expired
  if (promotion.endDate < now) {
    throw new BadRequestError('Mã khuyến mãi đã hết hạn');
  }

  // Check usage limit
  if (promotion.usedCount >= promotion.usageLimit) {
    throw new BadRequestError('Mã khuyến mãi đã hết lượt sử dụng');
  }

  // Check minimum order value
  if (orderTotal < promotion.minOrderValue) {
    throw new BadRequestError(`Đơn hàng chưa đạt giá trị tối thiểu ${promotion.minOrderValue.toLocaleString('vi-VN')}đ để áp dụng mã này`);
  }
